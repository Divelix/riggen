...
- [ ] **[3]** Step 2 — **the corpus joins the `mujoco` job, and the round
...
- ⚠ OPEN: **does step 2 fix `arm/thing.msh`?** (human, by step 2.)
  Recommendation: **yes**, as part of the step. It is a single triangle,
  MuJoCo refuses fewer than four vertices, and plans/actuator-table's
  acceptance run worked around it by hand; this plan's acceptance runs
  that corpus in CI, so the workaround would have to become permanent or
  the fixture becomes a tetrahedron. It changes mesh bytes only — every
  pose in that model is written explicitly.
