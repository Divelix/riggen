...
| [0008](0008-export-conventions.md) | Export conventions: meshes baked to meters as STL, `fullinertia`, a headless CLI export | Accepted, `fullinertia` narrowed by 0032 §4 (a static zero tensor is `diaginertia`) |
...
| [0014](0014-actuators-on-the-joint-mjcf-only-three-presets.md) | An actuator lives on its joint, is MJCF-only, is named after the joint, and comes in three presets; amends 0004 §4 | Accepted, amended by 0023, 0024 and 0027 |
| [0015](0015-mjcf-import-subset-and-one-import-vocabulary.md) | MJCF import reads the subset the document can hold; `<default>` is resolved, not stored; one import vocabulary with URDF | Accepted, §5's composite-joint bullet re-affirmed by 0022, §5's composition bullet amended by 0026, §7's round-trip sentence amended by 0032 (§7's decision stands) |
...
| [0019](0019-the-wheel-is-claimable-and-a-drag-keeps-the-hover-pick.md) | The wheel can be claimed by a rotate ring, and a gizmo drag keeps the hover pick; amends 0010 §3 and 0018 §3 | Accepted, §5 amended by 0029 |
...
| [0021](0021-two-modes-and-edit-is-the-zero-configuration.md) | The window has two modes, `Tab` between them; View poses through the joint tree and the glyphs alone, Edit is the zero configuration; supersedes the per-tool `q` reset | Accepted, amended three times 2026-09-05: §4 (an all-fixed document opens in View too), §1/§6 (a hidden thing answers nothing) and zen is orthogonal to the mode (`Esc` leaves it, `chrome_rects` empty); §1 narrowed by 0027 |
...
| [0027](0027-views-glyph-is-what-view-draws-and-the-band-is-opaque.md) | In View a joint glyph is the band or the bars, the tick and a filled bore for a driven joint — nothing else; the band's three shades are opaque; what View draws is what View answers; amends 0014 and narrows 0021 §1 | Accepted |
| [0028](0028-navigation-is-one-turntable-the-cube-aims-it-the-keys-move-its-pivot.md) | Navigation is one turntable: the ViewCube (app-side chrome, RoboCAD's, bottom-right) aims it and owns the projection readout, `W A S D E Q` translate its pivot along the camera's own basis, and the pivot is drawn without a fade while a camera gesture is live | Accepted, §3 amended by 0030 and 0031 |
| [0029](0029-a-rotate-drag-lands-a-frame-axis-on-a-feature.md) | A rotate drag lands one of the dragged frame's own axes on the feature under the cursor: the ring's plane, the four signed directions perpendicular to it, the nearest by angle, a direction-only ladder (circle > point), unconditional, a cyan spoke and an axis-prefixed readout; amends 0019 §5 | Accepted |
| [0030](0030-the-viewcube-names-the-axes-and-steps-the-view.md) | The ViewCube carries the world axes on its (−X, −Y, −Z) corner with letters, over a 50 % fill a hidden arm shows through, a letter dropped while its end is behind the cube; four arrows step the view 15° in yaw or pitch, no roll; the viewport's triad and its surface are removed and zen has no axes indicator; amends 0028 §3 | Accepted, §6's placement amended by 0031 |
| [0031](0031-the-viewcube-takes-the-top-right-the-visibility-row-its-left.md) | The ViewCube's block takes the top-right corner, where every CAD tool keeps it, and the visibility row ends 8 pt left of it; the bottom-right is empty; amends 0028 §3 and 0030 §6 | Accepted |
| [0032](0032-the-export-gate-guards-what-a-simulator-reads.md) | The export gate guards what a simulator reads: a static link with nothing to weigh it by gets no `<inertial>` and is named, a moving one without mass is refused with the fix, a static exactly-zero tensor is written `diaginertia="0 0 0"`; amends ADR-0015 §7's round-trip sentence | Accepted |
