...
| [0028](0028-navigation-is-one-turntable-the-cube-aims-it-the-keys-move-its-pivot.md) | Navigation is one turntable: the ViewCube (app-side chrome, RoboCAD's, bottom-right) aims it and owns the projection readout, `W A S D E Q` translate its pivot along the camera's own basis, and the pivot is drawn without a fade while a camera gesture is live | Accepted |
