...
class Robot:
    ...
    def reparent(
        self, link: int, new_parent: int, *, keep_world_pose: bool = False, q: dict[int, float] | None = None
    ) -> None: ...
    def set_root(self, link: int) -> None: ...
    def set_link_material(self, link: int, material: str | None) -> None: ...
    def assign_material_to_unweighed(self, material: str) -> list[int]:
        """Every link with visuals, no material and an inertial that needs a
        density it has not got gets ``material``, in one edit; returns their
        ids, in link order (ADR-0032 §5)."""
