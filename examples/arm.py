...
def build() -> riggen.Robot:
    ...
    # One coupled degree of freedom: the forearm follows the upper arm, so
...
def main() -> None:
    ...
    for path in robot.export(args.out, format=args.format, fk_samples=True):
        print(path)
    # `fore_joint` follows `upper_joint`; whatever is put in its own slot
    # is ignored.
    q: dict[str | riggen.Joint, float] = {"upper_joint": 0.5}
